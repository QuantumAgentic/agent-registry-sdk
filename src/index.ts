...
export function deriveAgentPda(
  creator: PublicKey,
  programId: PublicKey = AGENT_PROGRAM_ID
): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [AGENT_SEED, creator.toBuffer()],
    programId
  );
}

export function deriveStakingPoolPda(
  agentPda: PublicKey,
  programId: PublicKey = AGENT_STAKING_PROGRAM_ID
): [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [STAKING_POOL_SEED, agentPda.toBuffer()],
    programId
  );
}
...
export function deriveStakeAccountPda(
  ...
  return PublicKey.findProgramAddressSync(
    [STAKE_ACCOUNT_SEED, staker.toBuffer(), agentPda.toBuffer()],
    programId
  );
}
...
export function deriveTokenVaultPda(
  ...
  return PublicKey.findProgramAddressSync(
    [TOKEN_VAULT_SEED, poolPda.toBuffer()],
    programId
  );
}
...
function encodeOption<T>(value: T | null | undefined, encoder: (v: T) => Buffer): Buffer {
...
function encodeBytes(bytes: Uint8Array | number[]): Buffer {
 ...
 * Fetch agent by PDA
 */
export async function fetchAgentByPda(
  connection: Connection,
  agentPda: PublicKey,
  programId?: PublicKey
 ...
 */
export async function fetchAgentByCreator(
  connection: Connection,
  creator: PublicKey,
  programId?: PublicKey
...
/**
 * List all agent accounts for the program
 */
export async function listAgents(params: {
  connection: Connection;
  programId?: PublicKey;
  commitment?: Commitment;
}): Promise<Array<{ pda: PublicKey; account: AgentAccount }>> {
  const programId = params.programId || AGENT_PROGRAM_ID;
  const commitment = params.commitment || "confirmed";

  const accounts = await params.connection.getProgramAccounts(programId, { commitment });

  const result: Array<{ pda: PublicKey; account: AgentAccount }> = [];
  for (const acc of accounts) {
    try {
      const decoded = decodeAgentFromData(new Uint8Array(acc.account.data));
      result.push({ pda: acc.pubkey, account: decoded });
    } catch {
      // skip non-agent or unparsable accounts
    }
  }
  return result;
}
...
/**
 * List only active agents (isActive=true)
 */
export async function listActiveAgents(params: {
  connection: Connection;
  programId?: PublicKey;
  commitment?: Commitment;
}): Promise<Array<{ pda: PublicKey; account: AgentAccount }>> {
  const all = await listAgents(params);
  return all.filter((x) => x.account.isActive === true);
}
...
export async function hashCardJcs(card: unknown): Promise<Uint8Array> {
  const canon =
    typeof card === "string"
      ? (canonicalize as any)(JSON.parse(card))!
      : (canonicalize as any)(card as any)!;
...
export function makeConnection(
  rpcOrCluster?: string | ClusterName,
  commitment: Commitment = "confirmed"
): Connection {
  const url =
    rpcOrCluster && rpcOrCluster in RPC_BY_CLUSTER
